...
func main() {
	...
	r.Get("/v1/permits", handlers.ListPermits)
	r.Post("/v1/permits", handlers.CreatePermit)
	r.Get("/v1/permits/{id}", handlers.GetPermit)
	r.Patch("/v1/permits/{id}/status", handlers.UpdatePermitStatus)
	r.Post("/v1/permits/{id}/inspections", handlers.ScheduleInspection)
	r.Patch("/v1/permits/{id}/inspections/{iid}", handlers.UpdateInspection)

	r.Get("/health", healthHandler("permits"))
