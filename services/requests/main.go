...
func main() {
	...
	r.Get("/v1/requests", handlers.ListRequests)
	r.Post("/v1/requests", handlers.CreateRequest)
	r.Get("/v1/requests/{id}", handlers.GetRequest)
	r.Patch("/v1/requests/{id}/status", handlers.UpdateRequestStatus)
	r.Patch("/v1/requests/{id}/assign", handlers.AssignRequest)

	r.Get("/health", healthHandler("requests"))
